...
export const analyticsKeys = {
  ...
  sessionCompletion: (dateRange: DateRangeFilter) => ["analytics", "session-completion", dateRange] as const,
  ...
  distractions: (dateRange: DateRangeFilter, limit: number) => ["analytics", "distractions", dateRange, limit] as const,
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AnalyticsDashboardData> => {
      ...
      const [dailyStatsRaw, sessionsRaw, blockStatsRaw] = await Promise.all([
        invoke<any[]>("get_date_range_stats", { startDate, endDate }),
        invoke<Session[]>("get_session_history", { days }),
        invoke<any>("get_block_statistics", { days }).catch(() => null),
      ]);
      ...
      const focusTrend: FocusTrendDataPoint[] = dailyStats.map((stat) => ({
        date: stat.date as any,
        focusMinutes: Math.round(stat.totalFocusSeconds / 60) as any,
        sessions: stat.sessionsCompleted,
        completionRate: (stat.sessionsCompleted + stat.sessionsAbandoned > 0
          ? Math.round((stat.sessionsCompleted / (stat.sessionsCompleted + stat.sessionsAbandoned)) * 100)
          : 0) as any,
      ...
      const timeOfDay: TimeOfDayDistribution[] = Object.entries(hourlyData).map(([hour, data]) => ({
        ...
        focusMinutes: data.minutes as any,
        sessions: data.count,
        percentage: (totalMinutes > 0 ? Math.round((data.minutes / totalMinutes) * 100) : 0) as any,
      }));

      // Transform block statistics into distraction data
      const topDistractions: DistractionData[] = (blockStatsRaw?.top_blocked_items ?? []).map((item: any) => ({
        name: item.item_value,
        type: item.item_type === "app" ? "app" : "website",
      ...
      const averageProductivityScore =
        productivityScores.length > 0
          ? Math.round(productivityScores.reduce((sum, p) => sum + p.score, 0) / productivityScores.length)
          : 0;
      ...
      const mostProductiveHour =
        timeOfDay.length > 0 ? timeOfDay.reduce((max, h) => (h.focusMinutes > max.focusMinutes ? h : max)).hour : null;

      return {
        dateRange: { startDate: startDate as any, endDate: endDate as any },
        ...
        calendarHeatmap,
        summary: {
          totalFocusMinutes: totalFocusMinutes as any,
          totalSessions,
          averageSessionLength: averageSessionLength as any,
          completionRate: completionRate as any,
          totalDistractionsBlocked: blockStatsRaw?.total_attempts ?? 0,
          averageProductivityScore,
          currentStreak: calculateCurrentStreak(dailyStatsRaw),
          longestStreak: calculateLongestStreak(dailyStatsRaw),
          mostProductiveDay,
          mostProductiveHour,
        },
      };
...
function generateYearCalendar(year: number, stats: DailyAnalytics[]): CalendarHeatmapDay[] {
  ...
  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
    ...
    calendar.push({
      date: dateStr as any,
      focusMinutes: focusMinutes as any,
