// ai/providers/openai.rs - OpenAI provider using async-openai
...
impl OpenAiProvider {
    ...
    /// Create a new OpenAI provider with custom base URL (for Azure OpenAI)
    ...
    /// Build a chat completion request from our types
    fn build_request(
        &self,
        messages: &[Message],
...
impl LlmProvider for OpenAiProvider {
    ...
    async fn health_check(&self) -> Result<()> {
        ...
        self.client
            ...
            .list()
            .await
            .map_err(|e| Error::Network(format!("OpenAI health check failed: {}", e)))?;
    ...
    async fn complete(
        ...
        debug!(
            "Generating OpenAI completion with {} messages",
            messages.len()
        );
        ...
        let response = self
            ...
            .create(request)
            .await
            .map_err(|e| Error::Network(format!("OpenAI API error: {}", e)))?;
    ...
    async fn complete_stream(
        ...
        debug!(
            "Starting OpenAI streaming completion with {} messages",
            messages.len()
        );
        ...
        let mut stream = self
            ...
            .await
            .map_err(|e| Error::Network(format!("OpenAI streaming error: {}", e)))?;
